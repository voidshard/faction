...
func sqlFromFactionFilters(token string, in []*FactionFilter) (string, []interface{}, error) {
	...
	for _, f := range in {
		...
		if dbutils.IsValidID(f.ReligionID) {
			ands = append(ands, fmt.Sprintf("religion_id = $%d", len(args)+1))
			args = append(args, f.ReligionID)
		}
		if dbutils.IsValidID(f.GovernmentID) {
			ands = append(ands, fmt.Sprintf("government_id = $%d", len(args)+1))
			args = append(args, f.GovernmentID)
		}
		if dbutils.IsValidID(f.ParentFactionID) {
			ands = append(ands, fmt.Sprintf("parent_faction_id = $%d", len(args)+1))
	...
	return fmt.Sprintf(`SELECT id,
		...
		research_magic,
		research_occult
	    FROM %s %s ORDER BY id %s LIMIT $%d OFFSET $%d
	    `,
